...
# =============================================================================
# release.sh — Automates the full release pipeline for Cursy
#
...
#   9. Pushes the updated appcast.xml to the Cursy repository
#
# Usage:
...
GITHUB_REPO="ernestoespinosajr/cursy"

# Sparkle tools (auto-discovered from Xcode's SPM cache)
SPARKLE_BIN=$(find ~/Library/Developer/Xcode/DerivedData/Cursy*/SourcePackages/artifacts/sparkle/Sparkle/bin -maxdepth 0 2>/dev/null | head -1)
...
gh release create "${TAG}" "${DMG_PATH}" \
    ...
    --notes "Cursy v${MARKETING_VERSION}" \
    --latest

# ── Step 9: Push appcast.xml to the releases repo ───────────────────────────
# The appcast lives in the Cursy source repository. We clone it to a temporary
# directory, copy the new appcast into cursy-app, and push it.
...
cp "${PROJECT_DIR}/appcast.xml" "${RELEASES_REPO_DIR}/cursy-app/appcast.xml"
cd "${RELEASES_REPO_DIR}"
git add cursy-app/appcast.xml
