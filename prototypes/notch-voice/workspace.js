...
const preferences={language:'Español',annotation:'Cursor',model:'GPT-4.1',screen:true,spatial:true,cursor:true};
...
function workspaceMarkup(){
 let nav=section==='chats'?`<div class="side-heading">Chats${iconButton('plus','Nueva conversación de ejemplo','data-new')}</div><div class="chat-list">${chats.map((chat,index)=>`<button data-chat="${index}" aria-current="${index===selectedChat}">${chat.title}<small>Temporal</small></button>`).join('')}</div><div class="side-bottom"><p>Solo durante esta sesión</p><button data-section="general">${sf('gearshape')}Ajustes</button></div>`:
 `<button data-section="chats">${sf('chevron.left')}Volver a chats</button><div class="side-heading">Ajustes</div>${[['general','General','gearshape'],['cursor','Cursor','cursorarrow.motionlines'],['privacy','Privacidad','hand.raised']].map(([key,title,symbol])=>`<button data-section="${key}" aria-current="${section===key}">${sf(symbol)}${title}</button>`).join('')}<div class="side-bottom"><p>Vista previa · Sin cambios en la app</p></div>`;
...
function contentMarkup(){
 ...
 if(section==='general')return `<h2>General</h2><p>Tu experiencia, a tu manera.</p><div class="settings-list">${selectRow('language','Idioma',['Español','English'])}${selectRow('annotation','Indicación visual',['Cursor','Círculo','Flecha','Rectángulo'])}${selectRow('model','Modelo de respaldo',['GPT-4.1','GPT-4.1 mini'])}<div class="setting-row"><span>${sf('mic')} Mantén Control + Opción para hablar</span></div></div><p class="explanation">Controles de muestra. No cambian el modelo de voz ni el de localización.</p>`;
...
stage.addEventListener('click',event=>{
 ...
 const color=event.target.closest('[data-tint]');if(color){tint=color.dataset.tint;document.documentElement.style.setProperty('--cursor-tint',palette.find(item=>item[0]===tint)[2]);renderWorkspace()}
...
updateCursor=function(){
 ...
 if(['connecting','listening','processing'].includes(mode)){
